...
from model.losses import get_masked_ptc_loss, get_seg_loss, \
    CTCLoss_neg, DenseEnergyLoss, get_energy_loss
from utils import evaluate, imutils, optimizer
from utils.camutils import cam_to_label, cam_to_roi_mask2, multi_scale_cam2, \
    label_to_aff_mask, refine_cams_with_bkg_v2, crop_from_roi_neg
...
def validate(model=None, data_loader=None, args=None):
    ...
    Args:
        ...
        data_loader: The data loader for validation data.
        args: Additional arguments for validation.

    Returns:
        cls_score: Classification score.
        tab_results: Formatted results for evaluation.
    """
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            resized_cam_aux = F.interpolate(
                _cams_aux, 
                size=labels.shape[1:], 
                mode='bilinear', 
                align_corners=False)
            
            cam_label_aux = cam_to_label(
                resized_cam_aux, 
                cls_label, 
                bkg_thre=args.bkg_thre, 
                high_thre=args.high_thre, 
    ...
    tab_results = format_tabs([cam_score, cam_aux_score, seg_score],
                              name_list=["CAM", "aux_CAM", "Seg_Pred"],
...
def train(args=None):
    ...
    Args:
        args: Command line arguments containing configuration settings for training.
    """
    ...
    for n_iter in range(args.max_iters):
        ...
        cams, cams_aux = multi_scale_cam2(
            model, inputs=inputs, scales=args.cam_scales)
        
        roi_mask = cam_to_roi_mask2(
            cams_aux.detach(),
            cls_label=cls_label,
            low_thre=args.low_thre,
            hig_thre=args.high_thre)

        local_crops, flags = crop_from_roi_neg(
            images=crops[2], 
            roi_mask=roi_mask, 
            crop_num=ncrops-2, 
        ...
        refined_pseudo_label = refine_cams_with_bkg_v2(
            par,
            inputs_denorm,
            cams=valid_cam,
        ...
        # ptc loss
        resized_cams_aux = F.interpolate(
            cams_aux,
            size=fmap.shape[2:],
            mode="bilinear",
            align_corners=False)
        ...
        cls_score = evaluate.multilabel_score(
            cls_label.cpu().numpy()[0], 
            cls_pred.cpu().numpy()[0])
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info(
                    ...
                    "cls_loss: %.4f, cls_loss_aux: %.4f, "
                    "ptc_loss: %.4f, ctc_loss: %.4f, seg_loss: %.4f", 
                    n_iter + 1,
                    delta, eta,
                    cur_lr,
